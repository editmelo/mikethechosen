...
function CreditColumn({
  ...
  return (
    <Reveal delay={delay} className="flex-1">
      <p className="text-[0.72rem] font-semibold uppercase tracking-[0.28em] text-gold-light">
        {title}
      </p>
      ...
      <ul className="mt-6 flex flex-col gap-4">
        {names.map((name) => (
          <li
            key={name}
            className="font-serif text-2xl text-cream/85 transition-colors hover:text-gold-light sm:text-[1.7rem]"
          >
            {name}
          </li>
        ))}
...
export default function Credits() {
  return (
    <section id="credits" className="scroll-mt-20 bg-ink py-24 sm:py-32">
        ...
        <Reveal>
          <SectionHeading
            ...
            title={
              <>
                In the room with{" "}
                <span className="text-gold-gradient">the legends.</span>
              </>
            }
          />
          <p className="mx-auto mt-5 max-w-2xl text-center text-[1.02rem] leading-relaxed text-cream/70">
